...
# -------------- BEGIN PLOTTING----------  
    def do_plots(self,m):
                ...
                plt.pcolormesh(self.meshes.Y*self.meshes.dx, self.meshes.distY, self.T,vmin=float(self.deck.doc["Materials"]["Material1"]["Initial Temperature"]), vmax=float(self.deck.doc["Processing Parameters"]["Temperature"]),cmap=self.deck.doc["Plot"]["Color Map"])
                ...
                plt.pcolormesh(self.meshes.Y, self.meshes.X, self.Dic,vmin=self.meshes.dic, vmax=1,cmap=self.deck.doc["Plot"]["Color Map"])
                plt.colorbar()
                self.fig.suptitle('time: {:.2f}'.format( m*float(self.deck.doc["Simulation"]["Time Step"])), fontsize=16)
